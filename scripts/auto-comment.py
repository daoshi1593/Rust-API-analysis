...
def get_commented_content(client, model, content, file_path,log_dir):
	"""调用API获取注释后的内容"""
...
10. 保持注释与实际代码功能一致
11. 使用中文注释
特别注意：只需要返回markdown代码块，除此以外什么都不需要，而且不要对源代码做出任何逻辑上的改变
以下是需要注释的代码:\n{content}"""
	try:
  ...
  #去除文件名中的特殊字符,包括.
		...
		# 生成日志文件，打印文件名，原代码，注释后的代码
		with open(f'{log_dir}/{file_path}.log', 'w', encoding='utf-8') as file:
			file.write(f"文件名: {file_path}\n")
			file.write(f"原代码:\n{content}\n")
			file.write(f"注释后的代码:\n{respsonseContent}\n")
		# 提取注释后的代码
...
def main():
	...
	parser.add_argument('--model', default='deepseek-v3-241226', help='要使用的模型名称')
 	#os.environ.get("MODEL_NAME")
	parser.add_argument('--ignore-dirs', nargs='+', default=[], help='要忽略的目录，例如: venv node_modules')
	parser.add_argument('--log-dir', default='./log', help='日志文件目录')
	...
	for file_path in file_paths:
		...
		commented_content = get_commented_content(client, args.model, content,file_path,args.log_dir)
